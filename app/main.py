...
# ─────────────────────────────────────────────────────────────────────────────
# Lets you run this file directly to push the COE test transcript through
# the EXACT SAME validation + analysis path /upload-analyze uses — no
# FastAPI server, no HTTP call, no uvicorn.
#
...
#
# Change which file gets analyzed by editing TEST_JSON_PATH below. Absolute
# paths are used as-is; relative paths are resolved against app/.
# ─────────────────────────────────────────────────────────────────────────────

TEST_JSON_PATH = "/home/ai/Workspace/Fayrouz/QA-for-call-center/coe_test_2.json"
...
async def _run_local_test(json_path: Path) -> None:
    """Run one local COE transcript through the production QAAgent path."""
    print(f"\n{'=' * 70}\nLoading COE test transcript: {json_path}\n{'=' * 70}")
    ...
    # Preview the real deterministic COE gate before the full LLM-backed run.
    from app.service_hub.coe_validation import classify_coe_trigger

    trigger = classify_coe_trigger(payload)
    print("coe trigger preview:")
    ...
    if result.coe_validation:
        print("coe_validation (app.service_hub):")
        print(json.dumps(result.coe_validation, indent=2, ensure_ascii=False))
    else:
        print("coe_validation : None (the COE gate did not trigger)")
