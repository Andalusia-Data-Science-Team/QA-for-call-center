...
async def aggregate_results(state: AgentState) -> dict:
    ...
    merged: dict = {
        ...
        # Non-punitive doctor quality notices from _build_doctor_compliance_flags.
        # QAAnalysisResult (app/models/output.py) does not declare a
        # doctor_warnings field, so Pydantic's default extra="ignore" drops
        # this key when `merged` is validated below — it survives only in
        # this raw `merged`/state["parsed_data"] dict, never on state["result"].
